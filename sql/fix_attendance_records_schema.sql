-- Fix attendance_records table to match API expectations
-- This script adds missing columns and modifies existing ones
...
-- 3. Update verification_method constraint to match our API methods
ALTER TABLE attendance_records 
...
-- 7. Add unique constraint for employee_id + date (alternative to user_id + date)
-- First, let's see if we have conflicts
DO $$
BEGIN
    ...
    EXCEPTION WHEN unique_violation THEN
        -- There are duplicate records, let's handle them
        RAISE NOTICE 'Found duplicate employee_id + date records, cleaning up...';
        
        -- Keep only the latest record for each employee_id + date combination
...
-- 9. Show some sample data to verify
SELECT 'Updated attendance_records table:' as message;
SELECT 
...
-- 10. Show verification methods in use
SELECT 
    verification_method, 
    COUNT(*) as count 
FROM attendance_records 
